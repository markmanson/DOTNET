...
namespace Day8_9
{
    ...
    public class Common
    {
        ...
        public SqlConnection Connect = new SqlConnection(ConfigurationManager.ConnectionStrings[Constant.strConnect].ConnectionString);
        #region Open_Con
        /*************************************************************************************************************
        ...
        public ArrayList Log(string User, string Pass)
        {
            ...
            if (Cmpr_User(User))                                                                //CALL THE FUNCTION CMPR_USER
            {
                ...
                {
                    strAuth = string.Empty;                                               //ASSIGN STRAUTH THE MESSEGE "WELCOME"
        ...
        #region Jap_Login
        /*************************************************************************************************************
         * FUNCTION NAME                                :::                     Jap_Login                
         * AUTHOR NAME                                  :::                     ANKIT SHARMA
         * DATE                                         :::                     19/09/2012
         * DESCRIPTION                                  :::                     TO VALIDATE THE USER(fOR JAPANESE USER)
         * **********************************************************************************************************/
        public int Jap_Login(string strName, string strPass)
        {
            ArrayList arrlstUser;                                       //DECLARATION OF THE OBJECT OF AN ARRAYLIST
            int flag = 0;                                                 //DECLARATION OF AN INT VARIABLE
            try
            {
                arrlstUser = new ArrayList();                           //INITIALIZATION OF AN OBJECT OF ARRAYLIST
                arrlstUser.Add(Constant.strUser1);                      //ADD STRING TO ARRAYLIST
                arrlstUser.Add(Constant.strUser2);                      //ADD STRING TO ARRAYLIST
                for (int i = 0; i < arrlstUser.Count; i++)
                {
                    string[] strDetails = new string[2];                //INITIALIZATION  OF STRING TYPE ARRAY 
                    strDetails = arrlstUser[i].ToString().Split(Constant.chrUndrScr);   //STORE THE I TH INEDX STRING INTO STRING ARRAY AFTER SPLITING BY '_' 
                    //USER NAME MATCHING************************************************************
                    if (strName == strDetails[0])
                    {
                        //PASSWORD MATCHING ********************************************************
                        if (strPass == strDetails[1])
                        {
                            flag = 1;
                            break;
                        }
